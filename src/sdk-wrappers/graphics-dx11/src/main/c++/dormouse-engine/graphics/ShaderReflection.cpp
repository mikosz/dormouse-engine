...
ShaderReflection::InputParameterInfos buildInputParameterInfos(
	...
	for (size_t i = 0; i < shaderDescription.InputParameters; ++i) {
		...
		fromIntegral(info.dataType, static_cast<std::underlying_type_t<decltype(desc.ComponentType)>>(desc.ComponentType));
		info.elements = essentials::numberOfBitsOn(desc.Mask);

		DE_LOG_DEBUG << "Shader input parameter " << i << ": " << info.semantic << "#" << info.semanticIndex;
...
ShaderReflection::Type buildTypeInfo(ID3D11ShaderReflectionType& typeInfo, size_t size) {
	...
	type.elementOffset = essentials::roundUpToMultipleOf(elementSize, 16);
...
ShaderReflection::Variable buildVariableInfo(
	...
	DE_LOG_DEBUG << "Shader constant buffer variable " << variableIdx << ": " << desc.Name;
...
ShaderReflection::ConstantBufferInfos buildConstantBufferInfos(
	...
	for (size_t bufferIdx = 0; bufferIdx < shaderDescription.ConstantBuffers; ++bufferIdx) {
		...
		DE_LOG_DEBUG << "Shader constant buffer " << bufferIdx << ": " << desc.Name;
...
ShaderReflection::ResourceInfos buildResourceInfos(
	...
	for (size_t resourceIdx = 0; resourceIdx < shaderDescription.BoundResources; ++resourceIdx) {
		...
		fromIntegral(info.type, static_cast<std::underlying_type_t<decltype(resourceDesc.Type)>>(resourceDesc.Type));
		info.name = resourceDesc.Name;
		info.slot = resourceDesc.BindPoint;
		assert(resourceDesc.BindCount == 1);
		fromIntegral(
			info.dimension,
			static_cast<std::underlying_type_t<decltype(resourceDesc.Dimension)>>(resourceDesc.Dimension)
			);
...
ShaderReflection::ShaderReflection(const void* shaderData, size_t shaderSize) {
	system::windows::COMWrapper<ID3D11ShaderReflection> reflectionData;
	checkDirectXCall(
